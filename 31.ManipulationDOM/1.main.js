...
// С помощью метода textContent мы можем получить текстовое содержимое DOM
...
// Также мы можем поменять текстовое содержимое элемента
console.log(
  firstParagraphElement.textContent =
    "Новое текстовое содержимое первого параграфа"
);

// Если мы хотим добавить текст, а не заменить его, используем оператор +=
console.log(
  firstParagraphElement.textContent += " Еще какой-то текст"
);

// Если мы заменяем textContent, то теряем все дочерние элементы.
// Мы буквально перезаписываем все содержимое.
console.log(
  boxElement.textContent = "Обновленный текст убирает все элементы HTML"
);
...
// С помощью outerHTML мы получаем разметку HTML, включая сам элемент
console.log(boxElement.outerHTML);

// Это не самый лучший способ добавления элементов в HTML, так как он достаточно
// медленный, потому что переписывает весь HTML код заново
